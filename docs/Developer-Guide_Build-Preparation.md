...
- **The supported** compilation environment is [Ubuntu Bionic 18.04 x64](http://archive.ubuntu.com/ubuntu/dists/bionic/main/installer-amd64/current/images/netboot/mini.iso) **only!** (**no other releases are supported!** It has to be exactly 18.04.x, otherwise default compiler versions might not match so if you are on an older Ubuntu release upgrade to 18.04. If you use a newer Ubuntu version start with 18.04.x from scratch),
