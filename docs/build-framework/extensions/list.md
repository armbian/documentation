...
Shrinks the final image's compressed footprint. Removes bulky rarely-needed firmware trees (`netronome`, `mrv`, `mellanox`), runs `zerofree` on the image's ext4 partitions after unmount so unused blocks compress away, and logs the largest directories via `du`. Partition discovery globs `${LOOP}p?`, so only single-digit partitions (`p1`-`p9`) are covered. Pulls in `fs-tools::zerofree`; enable it to produce smaller downloadable images.
...
Fetches the host sources needed to assemble Marvell Armada A3720/A8040 bootloaders. Clones `A3700-utils` (bschnei's fork, fixes modern-toolchain build errors), `mv-ddr-marvell` and CZ.NIC `mox-boot-builder` for the WTMI secure firmware, all pinned to fixed commits; `binaries-marvell` and `cryptopp` still track mutable branches, so firmware is only reproducible to the extent those branches are stable. Used by EspressoBin/MacchiatoBin bootloader builds.
